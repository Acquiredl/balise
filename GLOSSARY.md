...
| **Audit trail** | Append-only, hash-chained JSONL record of every finding's evidence and reasoning for one assessment run: each record seals the previous record's hash, so any deletion, edit, or reordering is detectable (`report.verify_audit_trail`). The product's signature feature; deliverables regenerate from it without rescanning. |
