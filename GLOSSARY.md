...
| **Verification trail** | Hash-chained JSONL record (format `balise-audit-trail/1`, `docs/VERIFICATION-TRAIL.md`) of every finding's evidence and reasoning: a genesis record binds the chain to its engagement and declares the record count; each record seals the previous record's hash. Guarantees are tiered and never blurred: edits, deletions, reordering and truncation are caught from the file alone; wholesale regeneration is caught against the head fingerprint in the delivered documents, and against the package's seals. The product's signature feature; deliverables regenerate from it without rescanning. |
| **Package** | The delivered whole of an engagement: rapport, sommaire, trail, `evidence/` archive, and the manifest — verifiable offline by anyone holding it (`balise verify`). |
...
| **Assessment id** | Permanent engagement identifier (UUID), minted at run start, committed in the trail genesis, printed in both report headers. Everything in the package points back to it. |
