...
(ADR 0003). Semantic checks (A2, A4, A6, A8, A9) live in semantic.py.
"""
...
def check_a1_privacy_policy(site: SiteSnapshot) -> Finding:
    ...
    if policy_page is not None:
        return Finding("A1", Status.MET,
                       evidence=[policy_page.url],
    ...
    if link_hits:
        ...
        return Finding("A1", Status.PARTIAL,
                       evidence=[page.url, snippet],
                       reasoning="Privacy wording found but no dedicated policy page "
                                 "was retrievable at conventional paths.")
    return Finding("A1", Status.NOT_MET,
                   evidence=[p.url for p in site.pages],
                   reasoning="No privacy policy found on the retrieved pages "
                             "(s. 8.2 requires publication when PI is collected "
...
def check_a3_officer(site: SiteSnapshot) -> Finding:
    ...
    if not hits:
        return Finding("A3", Status.NOT_MET,
                       evidence=[p.url for p in site.pages],
                       reasoning="No mention of a privacy officer (responsable de la "
                                 "protection des renseignements personnels) on the "
                                 "retrieved pages (s. 3.1 requires title and contact "
                                 "information to be published).")
    page, snippet = hits[0]
    has_contact = bool(re.search(r"mailto:|@|téléphone|telephone|phone", page.html.lower()))
    if has_contact:
        return Finding("A3", Status.MET, evidence=[page.url, snippet],
                       reasoning="Officer mention with contact means on the same page.")
    return Finding("A3", Status.PARTIAL, evidence=[page.url, snippet],
...
def check_a5_trackers(site: SiteSnapshot) -> Finding:
    ...
    if root is None:
        return Finding("A5", Status.UNKNOWN, reasoning="No pages retrieved.")
    ...
    banners = [name for name, sigs in CONSENT_BANNER_SIGNATURES.items()
               if any(re.search(s, html) for s in sigs)]
    if not trackers:
        return Finding("A5", Status.MET, evidence=[root.url],
                       reasoning="No known tracker signatures in the initial payload. "
                                 "(Static analysis only; runtime behavior not observed.)")
    if banners:
        return Finding("A5", Status.PARTIAL,
                       evidence=[root.url, f"trackers: {', '.join(trackers)}",
                                 f"consent platform: {', '.join(banners)}"],
                       reasoning="Trackers present alongside a consent platform; whether "
                                 "they stay inactive before consent requires runtime "
                                 "verification. CAI Guidelines 2023-1 expect off-by-default.")
    return Finding("A5", Status.PARTIAL,
                   evidence=[root.url, f"trackers: {', '.join(trackers)}"],
                   reasoning="Tracker scripts in the initial payload with no consent "
                             "platform detected. Indicator of activation before consent "
                             "(regulator expectation, contested tier — see check note); "
...
def check_a7_french(site: SiteSnapshot) -> Finding:
    root = site.pages[0] if site.pages else None
    if root is None:
        return Finding("A7", Status.UNKNOWN, reasoning="No pages retrieved.")
    html = root.html
    ...
    evidence = [root.url, f"html lang={lang or 'absent'}",
                f"fr alternate={'yes' if has_fr_alternate else 'no'}"]
    if lang.startswith("fr") or looks_french:
        return Finding("A7", Status.MET, evidence=evidence,
                       reasoning="Site presents in French (Charter s. 52 exposure low). "
                                 "Policy/ToS French versions still need page-level review.")
    if has_fr_alternate:
        return Finding("A7", Status.PARTIAL, evidence=evidence,
                       reasoning="French alternate declared; completeness of the French "
                                 "version not verified.")
    return Finding("A7", Status.NOT_MET, evidence=evidence,
